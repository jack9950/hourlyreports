...
from getMissingDEPPs import get_missing_DEPPs, get_missing_DEPPs_breakdown
...
try:
  ...
  i = 0
  while (not os.path.isfile(homeFolder + 'report.csv') and i < 60):
    i+=1
    print('i = ', i)
    time.sleep(2)
...
for sale in DEPP_sales_all:
    ...
    # Add one more for Double DEPP
    if sale[3] == "Electric Repair Essentials & Surge Protection Plan 10.99 ":
        DEPP_sales.append(sale[0])
...
newName = homeFolder + 'report_MTD' + fileNameDate + '_' + fileNameTime +'.csv'
